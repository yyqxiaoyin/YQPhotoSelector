...
-(void)setUpViews{
    ...
    self.collectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT - 44) collectionViewLayout:flowLayout];
    ...
    self.totalNumLabel.text = [NSString stringWithFormat:@"%lu 张照片",self.dataSource.count];
...
-(void)selectPicBtn:(UIButton *)sender{
    ...
    if (sender.selected == NO) {//没有选择的时候
        ...
        if (self.selectArray.count+ 1 > self.maxCount) {
            ...
            [YQPhotoSelectorHUD showHUDWithTitle:[NSString stringWithFormat:@"最多只能选择%lu张图片",self.maxCount]];
        }else{
